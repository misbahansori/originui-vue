...
<template>
  <div class="space-y-4">
    ...
    <Accordion type="single" collapsible class="w-full" :default-value="'3'">
      ...
      >
        <AccordionTrigger
          class="items-center justify-start gap-3 rounded-md py-2 text-[15px] leading-6 font-semibold transition-all outline-none hover:no-underline focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-50 [&>svg]:-order-1 [&>svg>path:last-child]:origin-center [&>svg>path:last-child]:transition-all [&>svg>path:last-child]:duration-200 [&[data-state=open]>svg]:rotate-180 [&[data-state=open]>svg>path:last-child]:rotate-90 [&[data-state=open]>svg>path:last-child]:opacity-0"
        >
          {{ item.title }}
          <template #icon>
            <LucidePlus
